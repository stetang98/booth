...
> 提交前置：视频先传 YouTube（unlisted 即可），拿到链接后把下文所有 `【YOUTUBE链接】` 替换掉再开始粘贴。
...
|---|---|
| BUIDL name | `Booth` |
| Logo | 上传 `submission/booth-logo.png`（480×480） |
| Is this BUIDL an AI Agent? | **No**（开关关闭） |
| Category | Crypto / Web3 |
| └ Key innovation domains（多选） | **ZK**（必选）· 列表里若有 DAO / Governance / Infra 可加一个，没有就只留 ZK |
| Infrastructures deployed | 输入并选 **Stellar**（其余留空） |
| GitHub | `https://github.com/stetang98/booth` |
| Project website | `https://booth-stellar.vercel.app` |
| Demo video | `【YOUTUBE链接】` |
| Social link | `https://x.com/Stetang3438` |

**Vision（≤256 字符，粘这一行，242 字符）:**

The secret ballot, on-chain: anonymous voting for Stellar organizations. Ballots are admitted only by Groth16 proofs verified with Stellar's new BN254 host functions — anonymous yet eligible voters, double votes impossible, auditable tallies.
...
- 加分项：工具栏 🎬 图标可**内嵌** demo 视频，粘 `【YOUTUBE链接】`，评委不用跳走就能看。
...
▶ Demo: 【YOUTUBE链接】 · Live dApp (no wallet needed): https://booth-stellar.vercel.app · Code (MIT): https://github.com/stetang98/booth
...
# 提交前 5 分钟自检
================================================================
- [ ] 无痕窗口打开三链接全部可用：GitHub / booth-stellar.vercel.app / YouTube 视频
- [ ] dApp 上点 "Start a fresh demo poll" 投一票走通（顺手确认服务正常）
- [ ] 所有 `【YOUTUBE链接】` 已替换（README 里的视频链接我来回填）
- [ ] Vision 一行粘贴后没被截断（242 字符 < 256 上限）
